...
Summary_path = r'C:\Users\91604\Desktop\lungSeg\2D_UNet\runs\allpieces'
...
class MyTransform:
    ...
    def __call__(self, image, label, mode):   # image形状
        if mode == 'train':
            ...
            image = np.rot90(image, k=int(angle // 90), axes=(0, 1)).copy()  # 旋转，axes根据要迁移的轴进行选择
            label = np.rot90(label, k=int(angle // 90), axes=(0, 1)).copy()

        # 3. 将ndarray转换为tensor
        # 注意需要将tensor形状弄成一样的（使用reshape()）
        image = transforms.ToTensor()(image).reshape(1, 32, 56)
        label = transforms.ToTensor()(label).reshape(1, 32, 56)
        return image, label
...
def train(config):
    ...
    model.to(device)
    # 定义日志记录器
    logger = initLogger(config['model_name'])
    # 损失函数为交叉熵损失函数
    # 设置类别权重
    weights = torch.tensor([0.2, 1])  # 背景权重低，结节权重高
    criterion = nn.CrossEntropyLoss(weight=weights).to(device)

    # 对数据图像进行预处理
    transform = MyTransform()

    # 接下来读取图片和掩码，进行预处理。读取的结果已经是张量。
    dst_train = readDataList.UnetDataset(transform=transform, images_dir=config['trainPic_dir'], labels_dir=config['trainLab_dir'])
    # 将上面方法的实例传入DataLoader，得到适合模型训练的数据类型
    dataloader_train = DataLoader(dst_train, shuffle=True, batch_size=config['batch_size'])

    dst_valid = readDataList.UnetDataset(transform=transform, images_dir=config['validPic_dir'], labels_dir=config['validLab_dir'])
    ...
    for epoch in range(config['num_epoch']):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            # 梯度信息归零
            optimizer.zero_grad()
            # 使用model预测结果。输出的[batch_size,channel,H,W]是张量
            output = model(data)
            ...
            correct, labeled, inter, unoin, TP, FP, TN, FN = metrics.eval_metrics(output, target, config['num_classes'])
            correct_sum += correct
            labeled_sum += labeled
            inter_sum += inter
            unoin_sum += unoin
            ...
            #newIoU = 1.0 * (TP_sum / (np.spacing(1) + TP_sum + FP_sum + FN_sum) + TN_sum / (np.spacing(1) + FP_sum + FN_sum + TN_sum)) / 2
            recall = TP / (TP + FN)
            diceScore = TP / (TP + 0.5*FP + 0.5*FN)

            Precision_sum = Precision_sum + pixelAcc
            recall_sum = recall_sum + recall
            dice_sum = dice_sum + diceScore
            # 下面设置训练中输出显示哪些信息
            tbar.set_description('TRAIN ({}) | Loss: {:.3f} | Acc {:.4f} mIoU {:.4f} recall {:.4f} diceScore {:.4f}'.format(
                epoch, loss_sum/((batch_idx+1)*config['batch_size']),
                Precision_sum / round, IoU.mean(), recall_sum / round, dice_sum / round))
