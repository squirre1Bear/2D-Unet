...
def eval_metrics(output, target, num_classes):
    ...
    #print("bac="+str(bac)+"  pre_bac="+str(pre_bac))

    # 求出图片中总的像素数。target>0是排除里面有误的数据
    pixel_labeled = (target>0).sum()
    pixel_correct = ((predict == target) * (target>0)).sum()
    lung_correct = ((predict == target) * (target == 2)).sum()
    # 求预测像素的大小
    predict = predict * (target>0).long()
    intersection = predict * (predict == target).long()
    # print(torch.max(intersection).item())

    TP = ((predict == target) * (target == 2)).sum()
    TN = ((predict == target) * (target == 1)).sum()
    FP = ((predict == 2) * (target == 1)).sum()
    FN = ((predict == 1) * (target == 2)).sum()
    ...
    '''
    # 计算直方图中每个类别的像素数量，每个变量都是长度为2的张量，表示为0、1像素的个数
    ...
    TP = np.round(TP.cpu().numpy(), 5)
    TN = np.round(TN.cpu().numpy(), 5)
    FP = np.round(FP.cpu().numpy(), 5)
    FN = np.round(FN.cpu().numpy(), 5)
    #pixacc = 1.0 * correct / (np.spacing(1) + labeld)
    #mIoU = (1.0 * inter / (np.spacing(1) + union)).mean()

    return correct, labeld, inter, union, TP, FP, TN, FN