...
# 设置分块的形状大小，24*32*56的大小差不多能涵盖80%以上的结节。
    ...
    for dicom_name in os.listdir(dataset_path):
        ...
        # print(cbbox)
        #
        # aa = max(cbbox[0].stop-cbbox[0].start, aa)
        # bb = max(cbbox[1].stop-cbbox[1].start, bb)
        # cc = max(cbbox[2].stop-cbbox[2].start, cc)
        # print(aa, bb, cc)
    ...
    # 不同切片的image大小不同，需要找出最大尺寸，padding成一样大小的。
        ...
        #print(cmask.shape)
        ii += 1
    # 返回是list，里面存的数据是相同大小（72*88*104）的图片image
